...
def overall_forecast() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    fig1.gca().set_title("Daily Units Sold - Forecast", fontsize=14)
    fig1.gca().set_xlabel("Date")
    fig1.gca().set_ylabel("Units Sold")
    fig1.tight_layout()
    fig1.savefig("forecast_units_plot.png", dpi=150)
    print("[SAVED] forecast_units_plot.png")

    # --- Plot: Revenue ---
    fig2 = m_rev.plot(fc_rev)
    fig2.gca().set_title("Daily Revenue (EUR) - Forecast", fontsize=14)
    fig2.gca().set_xlabel("Date")
    fig2.gca().set_ylabel("Revenue (EUR)")
    fig2.gca().yaxis.set_major_formatter(
        mticker.FuncFormatter(lambda x, _: f"€{x:,.0f}")
    )
    fig2.tight_layout()
    fig2.savefig("forecast_revenue_plot.png", dpi=150)
    ...
    fig3.tight_layout()
    fig3.savefig("forecast_components.png", dpi=150)
    ...
    out.to_csv("forecast_results.csv", index=False)
...
def category_forecast():
    ...
    fig.tight_layout()
    fig.savefig("forecast_by_category.png", dpi=150)
...
def evaluate_model():
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig("forecast_evaluation.png", dpi=150)
    print("[SAVED] forecast_evaluation.png")

    comparison.to_csv("forecast_evaluation.csv", index=False)
